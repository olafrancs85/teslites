...
import { SpaceXDomains } from "@/components/spacex/domains";
...
--------------------------------- */
function computeSignalStrengthFromNarrative(
  narrative: string
): number {
...
function mapSignalEvolution(
  ...
  return profiles.map((p) => {
    const strength = computeSignalStrengthFromNarrative(
      p.narrativeTemplate
    );
...
--------------------------------- */
function deriveSecondOrderEffects(
  insights: Insight[]
): SecondOrderEffect[] {
...
/* ---------------------------------
   COMPONENT
--------------------------------- */
export default function SpaceXIntelligenceHub() {
  const [insights, setInsights] = useState<Insight[]>([]);
  const [loading, setLoading] = useState(true);

  const loadInsights = async () => {
    try {
      const res = await fetch("/api/teslite-ai/insights/spacex");
  ...
  const {
    ...
    regimeConfidence,
    regimePrediction,
  } = useMacroSignal(SpaceXDomains, computeSignalStrengthFromNarrative);

  /* ---------- UI ---------- */
  return (
    <section className="space-y-12 p-6">
      <section className="space-y-4">
        <h2 className="text-lg font-semibold">SpaceX Intelligence Signals</h2>
      ...
      <section className="space-y-6">
        {SpaceXDomains.map((domain: Domain) => (
